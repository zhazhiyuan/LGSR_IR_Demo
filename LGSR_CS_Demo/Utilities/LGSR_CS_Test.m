...
function [filename,subrate,alpha, beta, mu, c1, c2, jjj, PSN_Result,FSIM_Result, SSIM_Result,Time_s, Err_or]=LGSR_CS_Test(filename,IterNum,subrate,alpha, beta, mu, c1, c2)
    ...
    %    fprintf('Initial PSNR = %0.2f\n',csnr(par.Org,par.Initial,0,1));
        ...
        [reconstructed_image, PSN_Result,FSIM_Result,SSIM_Result, jjj, Err_or,All_PSNR]= LGSR_CS_Main(Y, Opts);
        ...
        Final_Name= strcat(filename,'_LGSR_CS_ratio_',num2str(subrate),'_PSNR_',num2str(PSN_Result),'_FSIM_',num2str(FSIM_Result),'_SSIM_',num2str(SSIM_Result),'.png');
            
        imwrite(uint8(reconstructed_image),strcat('./ratio_0.1_Results/',Final_Name));
        
       PSNR_Final_Name1= strcat(filename,'_LGSR_',num2str(subrate),'.txt');
